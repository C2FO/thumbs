...
$(function () {
    ...
    var AppView = thumbs.View.extend({
        ...
        initialize: function () {
            this._super("initialize", arugments);
            this.model.fetch();
        },

        //get format for cleared Todos
        formatClear: function () {
            var done = this.model.done().length;
            return "Clear " + done + " completed " + (done ? "item" : "items");
        },

        //get format for remaining Todos
        formatRemaining: function () {
            var remaining = this.model.remaining().length;
            return "<b>" + remaining + "</b> " + (remaining ? "items" : "item") + " left";
        },
        ...
        updateStats: function () {
            ...
            this.$clearCompletedA.html(this.formatClear());
            this.$todoCountDiv.html(this.formatRemaining());
            this.allCheckbox.checked = !this.model.remaining().length;
        },
        ...
        toggleAllComplete: function () {
            ...
            this.model.invoke("save", {'done': done});
        }

    });

    // Finally, we kick things off by creating the **App**.
    new AppView({model: Todos}).render();
